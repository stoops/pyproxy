	...
	if (klen >= MAXC):
		...
		while (x < mlen):
			...
			if (lval < ival):
				i = x ; break
			x += 1
		mins["d"].insert(i, (addr, lval))
		mins["d"] = mins["d"][:maxl+minl]
		mins["l"] = maxl
		...
		for fdes in rfds:
			if (fdes in srvs):
				...
				if (not addr in keyl):
					...
					fout("~ %s [%s:%s] (%s:%s)" % (prot, addr, dest, conn.fileno(), sock.fileno(), ))
