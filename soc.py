...
def inis(sock, dest):
	if (not fino(sock)):
		...
		if (not syns(temp, dest)):
			stdo("warn %s recv conn sock %s" % (self.name, dest, ))
			fins(temp, "inis serv conn sock")
		else:
			sock = temp
	return sock
...
class tcpc:
	def __init__(self, args):
		...
		self.smap = {}
		self.lsoc = None
		self.esoc = [None] * self.leng
		self.rsoc = [None] * self.leng
	...
	def recv(self, argp):
		...
		while (self.stat == 1):
			socs = []
			if (self.args.nots):
				if (fino(self.esoc[indx])):
					socs.append(self.esoc[indx])
			if (fino(self.rsoc[indx])):
				socs.append(self.rsoc[indx])
			if (not socs):
				if (not self.args.conz):
	...
	def send(self, argp):
		...
		while (self.stat == 1):
			...
			if (conn in rfds):
				...
				sock = self.esoc[indx] if (midx == 0) else self.rsoc[indx]
				tidx = ((indx * 2) + midx)
				sent(sock, mesg, tidx)
				lock = sobj["lock"]
				if ((secs - lock[0]) > 1):
		...
		while (self.stat == 1):
			...
			if ((secs - last) >= 5):
				for x in range(0, self.leng):
					...
					y = self.cons[x]["socs"]
					if (self.args.nots):
						self.esoc[y] = inis(self.esoc[y], reml)
					self.rsoc[y] = inis(self.rsoc[y], remo)
				last = secs
			keys = list(self.smap.keys())
			for adrk in keys:
				sobj = self.smap[adrk]
			...
			for idxk in keys:
				...
				if (not cobj):
					...
					if (self.args.conz):
						y = cobj["socs"]
						fins(self.esoc[y], "tcpc serv conn esoc")
						fins(self.rsoc[y], "tcpc serv conn remo")
			...
			if (self.lsoc in rfds):
				...
				try:
					...
					if (self.args.conz):
						socs = indx
					if (self.args.nots):
						self.esoc[socs] = inis(self.esoc[socs], reml)
					self.rsoc[socs] = inis(self.rsoc[socs], remo)
					if (not self.thrr[socs]):
						self.thrr[socs] = threading.Thread(target=self.recv, args=(socs, ))
						self.thrr[socs].start()
					cobj = { "stat":1, "socs":socs }
					(cobj["conn"], cobj["addr"]) = self.lsoc.accept()
					cobj["thro"] = threading.Thread(target=self.send, args=(cobj, ))
					cobj["thro"].start()
					self.cons[indx] = cobj
...
class udpc:
	...
	def serv(self):
		...
		while (self.stat == 1):
			...
			if ((secs - last) >= 5):
				if (self.args.nots):
					self.esoc = inis(self.esoc, reml)
				self.rsoc = inis(self.rsoc, remo)
				last = secs
			(rfds, wfds, efds) = sels([self.lsoc], 5)
			if (efds):
				break
...
def main():
	...
	argp.add_argument("-z", "--conz", action="store_true")
	...
	for x in range(0, (args.cons * 2) + 2):
		MUTX.append(threading.Lock())
